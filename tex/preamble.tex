\usepackage{amsmath, amsfonts, amssymb, amsthm}
% \usepackage[
% 	lmargin=3.7cm,%
% 	rmargin=2.5cm,%
% 	bmargin=2.5cm,%
% 	tmargin=2.5cm %
% ]{geometry}
\usepackage[paperwidth=20.32cm, paperheight=25.4cm]{geometry}
\usepackage{graphicx}
\usepackage{float}
\usepackage{apalike}
\usepackage{hyperref}
\usepackage{fancyhdr}
\usepackage{url}
\usepackage{algorithm}
\usepackage{algorithmic}
\usepackage{listings}
\usepackage{color}
\usepackage{sectsty}

\linespread{1.5}
% \setlength{\textwidth}{10cm}
% \setlength{\textheight}{15cm}


\chapterfont{\centering}


\definecolor{dkgreen}{rgb}{0,0.6,0}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.58,0,0.82}

\lstset{frame=tb,
	language=C++,
	aboveskip=3mm,
	belowskip=3mm,
	showstringspaces=false,
	columns=flexible,
	basicstyle={\small\ttfamily},
	numbers=none,
	numberstyle=\tiny\color{gray},
	keywordstyle=\color{blue},
	commentstyle=\color{dkgreen},
	stringstyle=\color{mauve},
	breaklines=true,
	breakatwhitespace=true,
	tabsize=3
}


%%%% ------ Links ------------
\hypersetup{
	colorlinks,
	linkcolor={red!50!black},
	citecolor={green!50!black},
	urlcolor={blue!80!black}
}

%%%%%% ------ Tikz Stuffs -----------------------
\usepackage{tikz}
\usepackage{tikz-cd}


%%%% ---------- Asymptote Stuffs ----------------
% \usepackage{asymptote}
% % \numberwithin{asy}{chapter}
% \renewcommand{\theasy}{\thechapter\Alph{asy}}
% \begin{asydef}
% 	// import extras;
% 	size(6cm);
% 	usepackage("amsmath");
% 	usepackage("amssymb");
% 	defaultpen(fontsize(11pt));
% 	settings.tex="latex";
% 	settings.outformat="pdf";
% \end{asydef}
% \def\asydir{asy}

% \graphicspath{ {./images} }

%%%%%% ---- Theorem Styles and environments ----------------------
\theoremstyle{definition}
\newtheorem{thm}{Theorem}[section]
\newtheorem{defn}{Definition}[section]
\newtheorem{prop}{Proposition}[section]
\newtheorem{lem}{Lemma}[section]
\newtheorem*{notn}{Notation}
\newtheorem{rem}{Remark}[section]


%%%%% ----- Custom Commands ----------------------
\newcommand{\ra}{\rightarrow} %% Shprt arrow to the right
\newcommand{\lra}{\longrightarrow}  %% long arrow to the right
\newcommand{\disp}{\displaystyle} %% for inline math displaystyle
\newcommand{\QQ}{\mathbb{Q}} %% Set of rational numbers
\newcommand{\NN}{\mathbb{N}} %% Set of natural numbers
\newcommand{\CC}{\mathbb{C}} %% Set of complex numbers
\newcommand{\PP}{\mathbb{P}} %% Set of complex numbers
\newcommand{\RR}{\mathbb{R}} %% Set of real numbers
\newcommand{\Sb}{\mathbb{S}}
\newcommand{\htp}[1]{\underset{#1}{\simeq}} %%% Homotopy of two functions in
\newcommand{\ZZ}{\mathbb{Z}}
\newcommand{\TT}{\mathbb{T}}
\newcommand{\KK}{\mathbb{K}}
\newcommand{\zdcl}{L_{dc}} %% The Zero-divisors-cup length
\newcommand{\tcomp}{\textsf{TC}} %% Topolocical complexity
\newcommand{\ltcomp}{\textsf{TC}^{LP}} %% Topolocical complexity
\newcommand{\abs}[1]{\lvert#1\rvert} %% Absolute valu of a value

%%%%% --------- Custom operators -------------------------------

\DeclareMathOperator{\rel}{rel}
\DeclareMathOperator{\map}{map}
\DeclareMathOperator{\cat}{cat}

%% Changes the Bibliography page header to `References`
\renewcommand{\bibname}{References}




\title{Topology of Robot Motion Planning Algorithms}
\author{\textbf{\textsc{Fawole Abdulrasheed Bolaji}} \\ Matric. No.: 207256 \\[.35cm] \textbf{Supervisor: Professor D.O.A Ajayi} \\ }
% \institute{University of Ibadan}
